...
def create_offset_handle():
    ...
    """
    offset_handle = cmds.spaceLocator("Offset_Handle")
    for axis in "XYZ":
        cmds.setAttr(
...
def create_proxy_setup(pivot_object, objects):
    ...
    cmds.delete(cmds.parentConstraint(pivot_object, offset_handle))
    cmds.makeIdentity(
        offset_handle, 
        apply=True, 
        translate=True,
    ...
    for object in objects:
        ...
        cmds.connectAttr(
            '{loc}.worldMatrix[0]'.format(loc=loc), 
...
cleanup_keys_from_data(flip_data)
